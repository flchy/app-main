      ...
      <div >
        <el-form v-loading="loading1" ref="form" :model="form" :rules="rules" label-width="100px" size="mini">
          <el-form-item label="名称" prop="tagName">
            <el-input v-model="form.tagName" placeholder="请输入名称" style="width:300px;" />
          </el-form-item>
  ...
  methods: {
    ...
    deleteColonel(id) {
      this.$confirm('此操作将永久删除', ' 是否继续?', '提示', {
        ...
        this.loading = true
        this.$store.dispatch('DeleteTag', { tagId: id }).then((res) => {
          if (res.code === 0) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    dateFormat: function(date) {
      ...
      this.$store.dispatch('TagAddOrUpdate', this.form).then((res) => {
        if (res.code === 0) {
          if (this.form.id) {
            this.$message('修改成功!')
          } else {
